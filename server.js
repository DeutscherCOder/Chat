...
// OpenRouter API ohne API-Key (mit anonymem Zugriff)
...
app.post('/api/messages', async (req, res) => {
    try {
        ...
        if (moderation.isViolation) {
            // Warnung als AI-Nachricht speichern
            await pool.query(
                'INSERT INTO messages (username, content, is_ai_warning) VALUES ($1, $2, $3)',
                ['🤖 Moderator', moderation.warning, true]
            );
            
            // Log in moderation_log
            await pool.query(
                'INSERT INTO moderation_log (username, violation_type, original_content) VALUES ($1, $2, $3)',
                [username, 'racism', content]
            );

            console.log(`⚠️ Nachricht von ${username} wurde moderiert`);
